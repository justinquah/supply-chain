...
- **D-13:** A **5th role, WAREHOUSE**, was added back after planning (user realized warehouse staff need access). WAREHOUSE = receiving team: records received qty + defects/shortages, marks fully-unloaded → RECEIVED. `0011` no longer remaps WAREHOUSE rows to ADMIN; WAREHOUSE added to the app role model. WAREHOUSE data grants (RLS) + receiving UI **deferred to Phase 4** (requirements WHS-01..04: goods receipt qty/damage remark + proof, WAREHOUSE marks RECEIVED, ETA notify, container arrived/unload timing — one PO = one container).
- **D-14:** A **6th role, LOGISTICS**, was confirmed as a real, distinct function (user clarified: LOGISTICS ≠ WAREHOUSE). LOGISTICS = clearance/freight: customs clearance, uploads **BL + K1_FINAL**, sets the **delivery-to-warehouse ETA** → PO **SHIPPED**. Originally the brief assigned this to SCM; reassigned to LOGISTICS. `0011` was corrected to **stop remapping LOGISTICS → ADMIN** (it is no longer in the remap `WHERE role IN (...)`); LOGISTICS added to the app role model. Its data grants + clearance UI (PO-04 actor change) **deferred to Phase 4**. Final role set: **SCM, ACCOUNTS, FINANCE, ADMIN, WAREHOUSE, LOGISTICS** (6). PO hand-off chain: SCM → Accounts → SCM → Finance → **Logistics (clearance → SHIPPED)** → **Warehouse (receive → RECEIVED)**.
