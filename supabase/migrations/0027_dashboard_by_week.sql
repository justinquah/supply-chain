-- ============================================================
-- Migration 0027 — Dashboard anchored on the STOCK UPLOAD WEEK (2026-07-07)
-- ============================================================
-- The dashboard time axis is now the weekly stock snapshot date, not the sales month.
-- product_dashboard_asof_date(p_date):
--   * STOCK  = latest snapshot per product on/before p_date (KL tz) — i.e. that week's data,
--              carrying forward the last known count for any product not in that week's upload.
--   * AMS    = avg over the 3 calendar months ending at the month of p_date (sales data).
--   * value/coverage use the primary-supplier cost + FX (as in 0022/0026).
-- Selecting the latest snapshot date = current inventory; older dates = that week's snapshot.
-- ============================================================
...
RETURNS TABLE (
  ...
  WITH wm AS (
    -- 3 calendar months ending at the month of p_date
  ...
  -- STOCK = latest snapshot per product on/before p_date (KL tz)
  stock AS (
...
-- Distinct stock-upload dates (KL tz) — powers the dashboard's week selector.
CREATE OR REPLACE VIEW stock_upload_weeks AS
SELECT DISTINCT (recorded_at AT TIME ZONE 'Asia/Kuala_Lumpur')::date AS snapshot_date
FROM stock_snapshots
ORDER BY 1;
